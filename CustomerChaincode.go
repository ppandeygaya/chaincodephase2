...
func (t *CustomerChaincode)  RegisterCustomer(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	CustomerObj.CUSTOMER=CustomerDataObj
	...
	CustomerDataList = append(CustomerDataList, CustomerObj)
...
func (t *CustomerChaincode)  GetCustomerDetails(stub shim.ChaincodeStubInterface, PAN_NUMBER string, AADHAR_NUMBER string) ([]byte, error) {
	...
	var CustomerTxObjects []Customer
	var CustomerTxObjects1 []Customer
	...
	if (PAN_NUMBER != ""){
		if ((obj.CUSTOMER.PAN_NUMBER) == PAN_NUMBER){
		...
		if ((obj.CUSTOMER.AADHAR_NUMBER) == AADHAR_NUMBER){
			CustomerTxObjects1 = append(CustomerTxObjects1,obj)
			//requiredObj = obj
			objFound = true
			break;
